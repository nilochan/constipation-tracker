...
const ConstipationReliefTracker = () => {
  ...
  // Load daily data function with error handling
  ...
  // Enhanced update daily data with better error handling and immediate feedback
  const updateDailyData = async (updates) => {
    ...
    try {
      ...
      // Reload analytics if on analytics tab to see immediate updates
      if (activeTab === 1) {
        const data = await ApiService.getAnalytics(30);
        setAnalyticsData(data);
      }
  ...
  const adjustWater = async (amount) => {
    ...
    await updateDailyData({ waterGlasses: newAmount });
  };
  ...
  const addBowelMovement = async (bristolType, urgency, straining, satisfaction) => {
    ...
    try {
      ...
      showNotification('Bowel movement recorded successfully');
  ...
  const removeBowelMovement = async (id) => {
    ...
    try {
      ...
      showNotification('Bowel movement deleted');
      ...
      showNotification('Failed to delete bowel movement', 'error');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <Dialog open={isOpen} onClose={onClose} maxWidth="md" fullWidth>
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={!selectedBristol || isSaving}
            variant="contained"
